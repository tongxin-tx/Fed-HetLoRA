...
def get_dataset(dataset_name, local_data_dir=None):
    if dataset_name in ["gsm8k"]:
    ...
    elif dataset_name == 'FinGPT/fingpt-fiqa_qa':
        dataset_name = local_data_dir + dataset_name if local_data_dir is not None else dataset_name
        ...
        # dataset = load_dataset("sst", trust_remote_code=True, split="train")
        print(dataset[0])
        ...
        dataset_name = local_data_dir + dataset_name if local_data_dir is not None else dataset_name
        print(dataset_name, local_data_dir)
        dataset = load_dataset(dataset_name, split="train")
...
def process_sft_dataset(dataset_name, dataset, dataset_sample):
    if dataset_name in ["lucasmccabe-lmi/CodeAlpaca-20k.dataset", "yahma/alpaca-cleaned",
                        "FinGPT/fingpt-sentiment-train", "FinGPT/fingpt-fiqa_qa"]:
        ...
        dataset = dataset.rename_column("output", "response")
    elif dataset_name in ['hf-datasets/WildChat']:
        dataset = dataset.filter(lambda x: x['toxic'] == False)
        dataset.to_parquet("hf-datasets/WildChat-nontoxic.parquet")
        dataset = dataset.remove_columns(['detoxify_moderation', 'toxic', 'redacted', 'openai_moderation', 'language'])

        # print(dataset[0])
    elif dataset_name in ['PKU-Alignment/BeaverTails']:

        dataset = dataset.filter(lambda x: x['is_safe'] == True)
        print('is_safe', dataset[0])
        dataset = dataset.rename_column("prompt", "instruction")
        dataset = dataset.remove_columns(['category', 'is_safe'])
    # elif dataset_name in ['FinGPT/fingpt-fiqa_qa']:
...
def process_mac_dataset(dataset):
    # dataset = dataset.map(alpaca_format, remove_columns=['category'], desc=f"Preprocessing dataset for unified format.")
...
def process_fenlei_dataset(dataset_name, dataset, dataset_sample):
    # if dataset_name in ["imdb","sst","sst2"]:
    #     dataset = dataset
    #
    # else:
    #     raise NotImplementedError(f"Dataset {dataset_name} is not supported.")
...
def mac_format(example):
    if example['input'] == "":
        example["prompt"] = example["prompt"]
    else:
        example["prompt"] = example["prompt"] + " " + example['input']
...
def my_process_dpo_dataset(dataset_name, dataset, template_name, dataset_sample):
    if dataset_name in ["Anthropic/hh-rlhf"]:
        dataset = dataset.map(partial(split_hh, template_name=template_name), load_from_cache_file=False)
    elif dataset_name in ["HuggingFaceH4/ultrafeedback_binarized"]:
        dataset = dataset.map(partial(split_ultrafeedback, template_name=template_name), load_from_cache_file=False)
