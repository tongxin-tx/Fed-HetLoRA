...
def get_clients_this_round(fed_args, round):
    if (fed_args.fed_alg).startswith('local'):
        ...
        if fed_args.num_clients < fed_args.sample_clients:
            ...
            # if 3 not in clients_this_round:
            #     # 随机选择一个索引来替换为 3
            #     replace_index = random.randint(0, len(clients_this_round) - 1)
            #     clients_this_round[replace_index] = 3
            #     # 再次排序以保持列表顺序
            #     clients_this_round = sorted(clients_this_round)
